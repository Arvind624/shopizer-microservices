...
import com.shopizer.catalog.model.catalog.category.Category;
...
public interface PageableCategoryRepository extends PagingAndSortingRepository<Category, Long> {
  ...
  @Query(value = "select distinct c from Category c "
  		...
  		+ "where c.merchantStore=?1 and cd.language=?2 and (?3 is null or cd.name like %?3%) "
  		+ "order by c.lineage, c.sortOrder asc",
      countQuery = "select  count(c) from Category c join c.descriptions cd where c.merchantStore=?1 and cd.language.id=?2 and (?3 is null or cd.name like %?3%)")
